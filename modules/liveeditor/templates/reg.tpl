...
<div class="head"><h5 class="iFrames"><span style="float:left; padding-left:10px; padding-right:10px;" class="itcen tpl_reg">{#LIVEEDITOR_REG_REG#}</span><form style="float:left !important;" class="form_registr" id="form_registr" action="index.php?do=modules&action=modedit&mod=liveeditor&moduleaction=reg&cp={$sess}" method="post"><input class="input_registr" name="input_registr" id="form_registr_name" type="text" value="" /><input class="basicBtn li_btn_Reg" value="{#LIVEEDITOR_DP_BTN#}" type="button"/>
</form>
{if $end_time_trial_lic[0]!=1}
{#LIVEEDITOR_DP_BTN_OR#}
<form class="form_delete" id="form_delete" action="index.php?do=modules&action=modedit&mod=liveeditor&moduleaction=reg&cp={$sess}" method="post"><input class="input_delete" name="input_delete" id="form_delete_name" type="hidden" value="delete" /><input class="basicBtn li_btn_Del" value="{#LIVEEDITOR_DP_BTN_NEXT#}" type="button"/>{else}{/if}</form>
