...
class Faq
{
	...
	public static function faqQuestionList($tpl_dir)
	{
		...
		$r_name = $AVE_DB->Query("SELECT * FROM ". PREFIX ."_modul_faq WHERE id = '" . $_GET['fid'] . "'")->FetchRow();
	...
	public static function faqShow($id)
	{
		...
		$faq = $AVE_DB->Query("SELECT faq_title, faq_description FROM " . PREFIX . "_modul_faq WHERE id = '" . (int)$id . "'")->fetchArray();
